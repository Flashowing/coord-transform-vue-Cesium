const showLoc = function (info) {
    this.constructor(info);
};
showLoc.prototype.coordinatesDiv = "";
showLoc.prototype.constructor = function (info) {
    let _this = this;
    if (!info.enabled) {
        return;
    }
    let _viewer = info.viewer;
    ...
    _this.coordinatesDiv.style.right = "40px";
    ...
    let cesiumViewer = document.getElementsByClassName('cesium-viewer');
    cesiumViewer[0].appendChild(_this.coordinatesDiv);
    _this.mouseMove(_viewer);
}
showLoc.prototype.mouseMove = function (_viewer) {
    let _this = this;
    _viewer.screenSpaceEventHandler.setInputAction(function onMouseMove(movement) {
            let lon, lat, MouseHeight, cameraHeight;
            //获取相机射线
            let ray = _viewer.scene.camera.getPickRay(movement.endPosition);
            //根据射线和场景求出在球面中的笛卡尔坐标
            let position1 = _viewer.scene.globe.pick(ray, _viewer.scene);
            //获取该浏览器坐标的顶部数据
            // let feature = _viewer.scene.pick(movement.endPosition);
            // console.log(feature)
            ...
            if (position1) {
                ...
                let cartesian = _viewer.scene.pickPosition(movement.endPosition);
            ...
            // 获取相机的高度
            cameraHeight = Math.ceil(_viewer.camera.positionCartographic.height);
            try {
